...
adds it to the Content Calendar project (#9) in the "Draft Posts" column.
...
> Set a **Publish Date** in the project and move this card to **To Be Published** to schedule social posts automatically.
"@
...
foreach ($f in $proj.fields.nodes) {
    ...
    switch -Wildcard ($n) {
        'status' {
            $statusFieldId = $f.id
            $opt = $f.options | Where-Object { $_.name -match 'draft' } | Select-Object -First 1
...
if (-not $draftOptionId) { Write-Error "No 'Draft Posts' option found in Status field." }
...
Write-Host ""
Write-Host "Done. Draft card created: $($issue.url)"
Write-Host "Next: Set a Publish Date in the project and move the card to 'To Be Published'."
